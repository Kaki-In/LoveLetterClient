...
class RoundGraphicLayer(GraphicLayer):
    def __init__(self, resources: _resources.Resources, players: list[_love_letter.LoveLetterPlayer], active_player: int, deck: _love_letter.LoveLetterDeck):
        ...
        for player in players:
            player_element = PlayerDisplayedElement(resources, player.get_card(), player.get_drawn_card(), player.get_name(), [i.get_character().get_value() for i in player.get_discard()])
            self._players.append(player_element)
            self._player_controllers.append(GraphicalPlayerController(player, player_element))
        
        self._deck = DeckDisplayedElement(resources, 0, None)
