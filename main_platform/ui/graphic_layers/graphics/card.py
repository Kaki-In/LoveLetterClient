...
class CardDisplayedElement(GameDisplayedElement):
    ...
    def paint(self, painter: _QtGui.QPainter, options: _QtWidgets.QStyleOptionGraphicsItem, widget: _QtWidgets.QWidget) -> None:
        ...
        if character is None:
            image_card = self._resources.get_themes_mapper().get_image_by_name("card").get_variant("back")
        
            card_painter.drawImage(_QtCore.QRectF(-w/2, -h/2, w, h), image_card)
        else:
            image_card = self._resources.get_themes_mapper().get_image_by_name("card").get_variant("front")
            image_name = "character_" + character.get_name().lower()
            
            card_painter.drawImage(_QtCore.QRectF(w * 5/100 - w/2, h * 10/100 - h/2, w * 90/100, h * 85/100), self._resources.get_themes_mapper().get_image_by_name(image_name).get_variant('cards'))
            card_painter.drawImage(_QtCore.QRectF(-w/2, -h/2, w, h), image_card)
